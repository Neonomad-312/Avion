...
const Header = ({ userAvatar }) => {
    ...
    return (
        <div className='header'>
            ...
            <div className='nav'>
                <div>
                    <img src="./src/assets/search.svg" alt="" style={{
                        marginRight: '16px'
                    }} />
                    <TranslationOutlined onClick={toggleLngMenu}/>
                    {lngMenuOpen && (
                        <div className="language-menu">
                            <button className={active === 'en' ? 'active' : ""} onClick={() => changeLng('en')}>EN</button>
            ...
            <ul className='navList'>
                {categories.map((categ) => (
                    <li key={categ.rout}><NavLink to={`/${categ.rout}`}>{t(categ.name)}</NavLink></li>
                ))}
            </ul>
